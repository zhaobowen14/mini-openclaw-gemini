...
SLIDE_CONTENT = {
    ...
    9: {
        ...
        "title": "量子計算を支える3つのコア原理",
        "subtitle": "Core Principles",
        "items": [
            "量子ビット\n従来の0か1だけでなく、両方の状態を同時に保持する最小単位",
            "重ね合わせ\n複数の状態が同時に存在する性質により、超並列計算を可能にする",
            "量子もつれ\n離れた粒子同士が互いに強い相関を持つ、高速処理の鍵となる現象"
        ]
    },
    10: {
        ...
        "subtitle": "Roadmap",
        "items": [
            "理論構築と初期検証\n量子アルゴリズムの開発と物理的なエラー検証",
            "ノイズ制限付き量子マシンの活用\nNISQデバイスを用いた特定分野での先行利用",
            "誤り訂正機能の実装\n計算エラーを自動で修正する論理量子ビットの確立",
            "汎用量子コンピュータの実現\n暗号解読や新素材開発などを可能にする完全なマシンの登場"
        ]
    }
}
...
def generate_presentation():
    ...
    print(f"Presentation saved to {output_path.absolute()}")
